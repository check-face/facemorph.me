...
const stages=new Set('asset-acquisition runtime-loading model-loading model-loaded mapping-loading mapping canary synthesis synthesis-complete alignment alignment-complete encoder-loading encoder-loaded encoder-correctness-check encoder-correctness-complete encoding encoding-complete mapping-complete original-cache-hit original-cached cache-unavailable codec-loading face morph export route-admitted'.split(' '));
const fields=new Set('schemaVersion session run event action platform language build stage elapsedMs stageMs errorCode browser provider device browserMajor bundle gpu routeOutcome'.split(' '));
...
const enums={event:['start','stage','completed','cancelled','failed'],action:['faces','morph','project'],errorCode:['cancelled','operation_failed'],platform:['ios','android','macos','windows','linux','other'],browser:['safari','chromium','firefox','other'],provider:['cpu','webgl','webgpu','native-cpu','native-gpu','auto'],gpu:['webgpu','webgl-only','none'],routeOutcome:['admitted','canary-failed','unsupported','start-failed','superseded']};
