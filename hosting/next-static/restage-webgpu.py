#!/usr/bin/env python3
"""Rebuild a staged runtime's WebGPU section from the sources the shipped engine expects.
...
def main():
    ...
    changed = {key: (before.get(key, {}).get('sha256'), value.get('sha256'))
               ...
               and before.get(key, {}).get('sha256') != value.get('sha256')}
