...
async function start(){
 ...
 const config=await(await fetch('./experiments-v22.json',{cache:'no-store'})).json();
 const cases={baseline:'best',candidate:'iphone-memory',cpu:'cpu-1'};const id=cases[query.get('case')];if(!id)throw Error('Specify case=baseline, candidate or cpu');
