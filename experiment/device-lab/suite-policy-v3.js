...
export function planSuite(config, device, environment){
 ...
 if(ios||affectedModel||!gpu){const order=['colour','ffmpeg','cpu-unshared-1','cpu-unshared-reload','iphone-memory','cpu-1','cpu-2','cpu-4','mobile-boundary-bounded','mobile-memory'];decisions.sort((a,b)=>(order.indexOf(a.id)<0?99:order.indexOf(a.id))-(order.indexOf(b.id)<0?99:order.indexOf(b.id)));}
