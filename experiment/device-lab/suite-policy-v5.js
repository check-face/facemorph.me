...
export function planSuite(config, device, environment){
 ...
 const match={affectedGpu:affectedModel&&gpu&&binding>=134217728,unsharedResearch:ios||!gpu||affectedModel,always:true,iosGpu:ios&&gpu&&binding>=134217728,gpu,large:!ios&&gpu&&binding>=134742528,bounded:!ios&&gpu&&binding>=134217728&&binding<134742528,threads2:threads>=2&&threads<4,threads4:threads>=4,weight1:threads<4};
 ...
 if(ios||affectedModel||!gpu){const order=['reference-diagnostic','colour','ffmpeg','cpu-unshared-1','cpu-unshared-reload','mobile-no-prepack','iphone-memory','cpu-1','cpu-2','cpu-4','mobile-boundary-bounded','mobile-memory'];decisions.sort((a,b)=>(order.indexOf(a.id)<0?99:order.indexOf(a.id))-(order.indexOf(b.id)<0?99:order.indexOf(b.id)));}
