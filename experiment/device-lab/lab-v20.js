import {runStatus,paintRunStatus} from './run-status-v1.js';
import {planSuite,resolveDecision} from './suite-policy-v1.js';
import {ResultStore} from './save-client-v4.js';
...
function execute(cfg){return new Promise(resolve=>{
 ...
 try{instance=new Worker(cfg.worker,{type:'module'});worker=instance;
  ...
  instance.onerror=e=>finish({completed:false,error:'Worker failed: '+e.message,workerError:{message:e.message,filename:e.filename,line:e.lineno,column:e.colno}});instance.onmessageerror=()=>finish({completed:false,error:'Worker message could not be decoded'});
...
async function run(){report=null;active=null;phase='starting';saveConfirmed=false;updateRunStatus();controls(true);halt=false;let vis,wake;
 try{
  ...
  const device=await diagnostics(config);const selection=planSuite(config,device,{crossOriginIsolated,hardwareConcurrency:navigator.hardwareConcurrency});const selected=selection.decisions.filter(d=>d.status!=='skipped').map(d=>config.experiments.find(e=>e.id===d.id));showPlan(selection);report={schemaVersion:1,...device,selection,preflightPolicy:config.policyVersion,fullQualification:config.automaticSuite.fullQualification===true,reverseOrder:false,automaticSuite:structuredClone(config.automaticSuite),suiteVersion:config.version,started:new Date().toISOString(),label:$('#label').value,userAgent:navigator.userAgent,hardwareConcurrency:navigator.hardwareConcurrency,deviceMemory:navigator.deviceMemory,secureContext:isSecureContext,crossOriginIsolated,screen:{width:screen.width,height:screen.height,dpr:devicePixelRatio},selected:selected.map(e=>e.id),experimentConfig:selected,visibility:[],results:[]};
...
async function init(){try{
 const response=await fetch('./experiments-v20.json',{cache:'no-store'});if(!response.ok)throw Error('Experiment configuration HTTP '+response.status);config=await response.json();
 ...
 try{const prior=JSON.parse(storage.get('facemorph-lab-last'));if(prior?.runId&&Array.isArray(prior.results)){report=prior;const ack=JSON.parse(storage.get('facemorph-lab-last-saved')||'null');saveConfirmed=ack?.runId===report.runId&&ack?.revision===report.revision;if(!report.finished){report.finished=new Date().toISOString();report.interrupted=true;report.resumeDiagnostics={navigationType:performance.getEntriesByType('navigation')[0]?.type||'unknown',documentWasDiscarded:document.wasDiscarded??null,resumedAt:new Date().toISOString()};saveConfirmed=false;for(const row of report.results)if(row.completed===undefined){row.completed=false;row.error='Previous tab closed or reloaded before completion';}report.revision=Math.max(report.revision||0,store.entries.get(report.runId)?.report.revision||0)+1;const pending=store.entries.get(report.runId);store.put({...pending,token:pending?.token??null,report:structuredClone(report)});persistLast();}render();}}catch{}
