import {ResultStore} from './save-client-v2.js';
import {diagnostics} from './diagnostics-v10.js';
...
function render(){if(!report)return;const results=report.results||[],synthesis=results.filter(r=>!['ffmpeg','colour'].includes(r.id));const passed=synthesis.filter(r=>r.completed===true&&!r.error);
 ...
 for(const row of results){const tr=document.createElement('tr');const valid=row.completed===true&&!row.error;const checks=row.checks||row.rows||[];const count=checks.filter(x=>x.passed===true).length;
  for(const value of [report.experimentConfig?.find(e=>e.id===row.id)?.name||row.id,valid?(row.encodeMs?row.encodeMs.toFixed(1)+' ms encode':row.singleMedianMs?row.singleMedianMs.toFixed(1)+' ms':row.medianInferenceMs?row.medianInferenceMs.toFixed(1)+' ms':'—'):'—',valid&&row.msPerFace?row.msPerFace.toFixed(1)+' ms/face':'—',outcome(row)+(checks.length?` · ${count}/${checks.length} reference checks`:''),row.error||row.progress||'']){const td=document.createElement('td');td.textContent=value;tr.append(td);}body.append(tr);
...
function controls(running){$('#run').disabled=running;$('#stop').disabled=!running;$('#skip').disabled=!running;for(const el of document.querySelectorAll('#cases input,#research-cases input,#defaults,#phone,#full,#reverse,#label'))el.disabled=running;}
async function run(){const selected=[...document.querySelectorAll('#cases input:checked,#research-cases input:checked')].map(c=>config.experiments.find(e=>e.id===c.value));if(!selected.length){$('#status').textContent='Select at least one experiment.';return;}if($('#reverse').checked)selected.reverse();controls(true);halt=false;let vis,wake;
 try{
  $('#status').textContent='Checking device and confirming automatic saving before downloading models…';
  const device=await diagnostics(config);report={schemaVersion:1,...device,preflightPolicy:config.policyVersion,fullQualification:$('#full').checked,reverseOrder:$('#reverse').checked,suiteVersion:config.version,started:new Date().toISOString(),label:$('#label').value,userAgent:navigator.userAgent,hardwareConcurrency:navigator.hardwareConcurrency,deviceMemory:navigator.deviceMemory,secureContext:isSecureContext,crossOriginIsolated,screen:{width:screen.width,height:screen.height,dpr:devicePixelRatio},selected:selected.map(e=>e.id),experimentConfig:selected,visibility:[],results:[]};
...
async function init(){try{
 const response=await fetch('./experiments-v10.json',{cache:'no-store'});if(!response.ok)throw Error('Experiment configuration HTTP '+response.status);config=await response.json();
 for(const e of config.experiments){const l=document.createElement('label');l.className='case';const c=document.createElement('input');c.type='checkbox';c.value=e.id;c.checked=e.default;l.append(c,document.createTextNode(e.name));(e.default?$('#cases'):$('#research-cases')).append(l);}
 ...
 $('#phone').onclick=()=>{const ids=new Set(['colour','mobile','mobile-124','cpu-1','cpu-2','cpu-weight-1','ffmpeg']);document.querySelectorAll('#cases input,#research-cases input').forEach(c=>c.checked=ids.has(c.value));$('#research').open=true;};$('#defaults').onclick=()=>document.querySelectorAll('#cases input,#research-cases input').forEach(c=>c.checked=config.experiments.find(e=>e.id===c.value).default);
 addEventListener('online',()=>store.flush());setInterval(()=>store.flush(),30000);controls(false);
 $('#status').textContent='Ready. Open this same page on any device—no key or account needed. Saving is checked before experiments start.';
