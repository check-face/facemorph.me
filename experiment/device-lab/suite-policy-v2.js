...
export function planSuite(config, device, environment){
 ...
 const facts={ios,gpu,bindingBytes:binding??null,effectiveCpuThreads:threads,classification:ios?'ios-conservative':!gpu?'cpu-only':binding>=134742528?'large-binding-gpu':binding>=134217728?'bounded-binding-gpu':'limited-or-unknown-gpu'};
 const match={always:true,iosGpu:ios&&gpu&&binding>=134217728,gpu,large:!ios&&gpu&&binding>=134742528,bounded:!ios&&gpu&&binding>=134217728&&binding<134742528,threads2:threads>=2&&threads<4,threads4:threads>=4,weight1:threads<4};
 ...
 if(ios){const order=['colour','ffmpeg','iphone-memory','cpu-1','cpu-2','cpu-4'];decisions.sort((a,b)=>(order.indexOf(a.id)<0?99:order.indexOf(a.id))-(order.indexOf(b.id)<0?99:order.indexOf(b.id)));}
