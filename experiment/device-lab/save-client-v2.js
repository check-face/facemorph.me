...
export class ResultStore{
 constructor({onStatus=()=>{},onRecovered=()=>{},fetcher=fetch,storage,timeoutMs=15000}={}){
 ...
 put(e){this.entries.set(e.report.runId,e);try{this.storage?.setItem(prefix+e.report.runId,JSON.stringify(e));}catch{this.onStatus('Local storage is unavailable; keep this tab open until the server confirms saving.',false);}}
 ...
 enqueue(report){const old=this.entries.get(report.runId);const token=old?.token||this.tokens?.get(report.runId);if(!token)throw Error('No credential for this run');report.revision=Math.max(report.revision||0,old?.report.revision||0)+1;this.put({token,report:structuredClone(report)});this.remember(report.runId,token);}
 remember(id,token){this.tokens??=new Map();this.tokens.set(id,token);}
