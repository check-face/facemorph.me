...
const width=256,height=256,fps=4;
self.onmessage=async({data})=>{
 const frames=data.frames?.length||0;
 const row={kind:'generated-face-ffmpeg-compatibility',source:'Three distinct cached full1024 generated originals resized to256',coreVersion:'0.12.10',threads:1,width,height,frames,fps,completed:false,logs:[]};
 let core;
 ...
 try{
  ...
  if(frames!==3||data.frames.some(x=>!(x instanceof ArrayBuffer)||x.byteLength!==width*height*4))throw Error('Expected three cached face frames');
  const input=new Uint8Array(width*height*4*frames);data.frames.forEach((x,i)=>input.set(new Uint8Array(x),i*width*height*4));data.frames=null;
  ...
  progress('Encoding 3 generated 256px frames to H.264 / MP4…');
  ...
  const sample=f=>{const values=[];for(let y=8;y<height;y+=16)for(let x=8;x<width;x+=16)for(let ch=0;ch<3;ch++)values.push(decoded[((f*height+y)*width+x)*4+ch]);return values;};row.playbackReference={first:sample(0),last:sample(frames-1),meanErrorLimit:8};
