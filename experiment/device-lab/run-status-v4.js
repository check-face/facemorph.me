// v4: interruption wording. Since per-case checkpoints and resume
// (recovery-resume-v1.js), an interrupted run is resumable; the banner tells the tester
// that instead of declaring the heavy tests lost.
export function runStatus({report,phase='idle',saved=false,now=Date.now()}){
 ...
 if(report?.finished){
  if(report.interrupted)return {kind:'warning',title:'INTERRUPTED — tests did not finish',detail:`The previous tab closed or reloaded. ${done} experiment(s) have partial or final results. Press Resume experiments to continue the same run; checkpointed stages are not repeated and recovered partials stay separate from fresh results. ${saved?'Interruption and results saved.':'Results retained here; save status shown below.'}`,done,total};
  if(report.stopped||report.error)return {kind:'warning',title:report.stopped?'STOPPED — tests did not finish':'RUN ENDED EARLY — tests did not finish',detail:`${done} of ${total} experiments finished · ${time}. ${saved?'Partial results saved.':'Partial results retained; check saving below.'} The finished experiments stay saved; you can start a new run.`,done,total};
