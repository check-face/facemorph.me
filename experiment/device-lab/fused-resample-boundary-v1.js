...
export async function fusedResampleBoundary(device,{phase,phaseB,demod,noise,filter,bias,output},m,tail=false,workgroup=256,variant='bounded'){
 const module=device.createShaderModule({code:boundaryCode(m,tail,variant,workgroup)}),info=await module.getCompilationInfo();if(info.messages.some(m=>m.type==='error'))throw Error(info.messages.map(x=>x.message).join('\n'));
 const pipeline=await device.createComputePipelineAsync({layout:'auto',compute:{module,entryPoint:'main'}}),used=tail?[[0,phase],[6,phaseB],[1,demod],[2,noise],[3,filter],[4,bias],[5,output]]:[[0,phase],[6,phaseB],[3,filter],[5,output]],group=device.createBindGroup({layout:pipeline.getBindGroupLayout(0),entries:used.map(([binding,buffer])=>({binding,resource:{buffer}}))});
