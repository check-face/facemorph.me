export function runStatus({report,phase='idle',saved=false,now=Date.now()}){
 ...
 if(report?.finished){
  if(report.interrupted)return {kind:'warning',title:'INTERRUPTED — tests did not finish',detail:`The previous tab closed or reloaded. ${done} experiment(s) have partial or final results. ${saved?'Interruption and results saved.':'Results retained here; save status shown below.'} Press Run again. On the same suite version, the interrupted test is skipped so the remaining tests can run.`,done,total};
