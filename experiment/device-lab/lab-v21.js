...
import {planSuite,resolveDecision,applyInterruptedRoutes} from './suite-policy-v2.js';
...
async function init(){try{
 const response=await fetch('./experiments-v21.json',{cache:'no-store'});if(!response.ok)throw Error('Experiment configuration HTTP '+response.status);config=await response.json();
