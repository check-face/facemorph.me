...
export class ResultStore{
 ...
 enqueue(report){const old=this.entries.get(report.runId);const token=old?.token||this.credential(report.runId);if(!token)throw Error('No credential for this run');report.revision=Math.max(report.revision||0,old?.report.revision||0)+1;this.put({...old,token,report:structuredClone(report)});this.remember(report.runId,token);}
 ...
 async recover(e,reason='Missing or expired previous write credential'){const old=e.report.runId,r=await this.reserve(e.report.test);const latest=this.entries.get(old)||e;const recovered={...latest,authRecoveryAttempted:true,token:r.writeToken,report:{...latest.report,runId:r.runId,...(reason.startsWith('Concurrent')?{conflictFromRunId:old}:{recoveredFromRunId:latest.report.recoveredFromRunId||old}),recoveryReason:reason,revision:1}};this.put(recovered);this.remember(r.runId,r.writeToken);this.remove(old);this.onRecovered(old,recovered.report);return recovered;}
 async importLegacy(){
 ...
 async flushAll(){
  try{
   while(this.entries.size){let e=this.entries.values().next().value;if(!e.token)e=await this.recover(e);const snapshot=e;
    ...
    catch(error){if(error.status===401&&!e.authRecoveryAttempted){await this.recover(this.entries.get(e.report.runId)||e);continue;}if(error.status===409&&!e.conflictRecoveryAttempted){const latest=this.entries.get(e.report.runId)||e;const recovered=await this.recover(latest,'Concurrent save conflict; local results preserved as a separate run');recovered.conflictRecoveryAttempted=true;this.put(recovered);continue;}throw error;}
