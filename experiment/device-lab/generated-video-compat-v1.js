...
export async function checkMP4(bytes,{width,height,frames,fps,playbackReference,signal,onProgress=()=>{}}){
 ...
 try{
  for(let attempt=0;attempt<2&&!signal?.aborted;attempt++){
   ...
   try{
    ...
    await new Promise((resolve,reject)=>{
     ...
     const next=(now,m)=>{
      ...
      try{
       ...
       if(first||last){ctx.drawImage(video,0,0);const rgba=ctx.getImageData(0,0,width,height).data,pixel=[];for(let y=8;y<height;y+=16)for(let x=8;x<width;x+=16)for(let ch=0;ch<3;ch++)pixel.push(rgba[(y*width+x)*4+ch]);if(first)row.firstPixel=pixel;if(last)row.lastPixel=pixel;}
    ...
    const mae=(actual,expected)=>{if(!expected||actual.length!==expected.length||!actual.length)throw Error('Missing playback reference');return actual.reduce((sum,x,i)=>sum+Math.abs(x-expected[i]),0)/actual.length;};row.firstMeanError=mae(row.firstPixel,playbackReference?.first);row.lastMeanError=mae(row.lastPixel,playbackReference?.last);if(row.firstMeanError>8||row.lastMeanError>8)throw Error('Presented face content mismatch');
