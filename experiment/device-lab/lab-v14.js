...
async function init(){try{
 const response=await fetch('./experiments-v14.json',{cache:'no-store'});if(!response.ok)throw Error('Experiment configuration HTTP '+response.status);config=await response.json();
