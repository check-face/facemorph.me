...
function render(){if(!report)return;const results=report.results||[],synthesis=results.filter(r=>!['ffmpeg','colour'].includes(r.id)&&!r.diagnosticOnly);const passed=synthesis.filter(r=>r.completed===true&&!r.error);
...
async function run(){controls(true);halt=false;let vis,wake;
 try{
...
async function init(){try{
 const response=await fetch('./experiments-v18.json',{cache:'no-store'});if(!response.ok)throw Error('Experiment configuration HTTP '+response.status);config=await response.json();
 ...
 $('#status').textContent='Ready. Press Run experiments and leave this tab visible. A curated suite runs automatically; saving is checked before starting.';
