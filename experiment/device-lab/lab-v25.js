...
import {planSuite,resolveDecision,applyInterruptedRoutes} from './suite-policy-v4.js';
import {ResultStore} from './save-client-v5.js';
import {diagnostics} from './diagnostics-v12.js';
import {checkMP4} from './video-compat-v4.js';
...
function render(){updateRunStatus();if(!report)return;const results=report.results||[];$('.table').hidden=results.length===0;const synthesis=results.filter(r=>!['ffmpeg','colour','reference-diagnostic'].includes(r.id)&&!r.diagnosticOnly);const passed=synthesis.filter(r=>r.completed===true&&!r.error);
...
function execute(cfg){return new Promise(resolve=>{
 const row={id:cfg.id,status:'Running',rows:[],started:new Date().toISOString()};active=row;report.results.push(row);const playbackController=new AbortController();let last=Date.now(),ended=false,timer,start=last,instance;
 const mergeCheckpoint=patch=>{const {completed,status,finished,...evidence}=patch;Object.assign(row,evidence);if(completed!==undefined)row.workerReportedCompleted=completed;if(status!==undefined)row.workerReportedStatus=status;};
 const finish=patch=>{if(ended)return;ended=true;playbackController.abort();if(row.playbackPending)row.playbackPending=false;clearInterval(timer);Object.assign(row,patch,{status:patch.completed?'Completed':'Failed',finished:new Date().toISOString(),elapsedMs:Date.now()-start});instance?.terminate();if(worker===instance)worker=null;cancelCurrent=null;save();resolve();};
...
async function init(){try{
 const response=await fetch('./experiments-v25.json',{cache:'no-store'});if(!response.ok)throw Error('Experiment configuration HTTP '+response.status);config=await response.json();
