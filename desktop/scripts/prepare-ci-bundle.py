...
def verified_source(directory):
    ...
    try:
        ...
        with urllib.request.urlopen(urllib.request.Request(SOURCE_URL,headers={'User-Agent':'FaceMorph-Preview/0.1 build-acquisition'}), timeout=120) as source, temporary.open('xb') as output:
