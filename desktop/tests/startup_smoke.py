...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='checkface launch space ') as folder:
            ...
            try:
                ...
                if process.poll() is None:
                    ...
                    try:
                        ...
                        process.kill()
                        process.wait(timeout=5)
        if evidence != {'rendered': True, 'missingRuntimeRejected': not args.require_runtime, 'runtimeAvailable': args.require_runtime}:
        ...
        print('PASS: actual Elmish DOM, IPC runtime state, unrelated working directory, bounded shutdown')
