...
def main():
    ...
    photo_case=manifest.get('photoCanary')
    if not photo_case:raise AssertionError('Independent photo canary missing')
    ...
    a.report.parent.mkdir(parents=True,exist_ok=True)
    a.report.write_text(json.dumps({'schemaVersion':1,'workerSha256':checksum(a.worker),'manifestSha256':checksum(a.manifest),'passed':True,'installedUiTested':False,'photoNumericalReferenceQualified':True,'photoMaxAbs':photo_error,'runs':reports},indent=2)+'\n')
