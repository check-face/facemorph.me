...
const MAX_REQUEST: usize = 36 * 1024 * 1024;
...
impl Bridge {
    ...
    pub fn release(&self) {
        ...
        // Window destruction must give the supervisor time to kill/reap its worker
        // before the application exits and its threads disappear.
        let deadline = Instant::now() + Duration::from_secs(2);
        while Instant::now() < deadline {
            if self.active.lock().map(|job| job.is_none()).unwrap_or(true) {
                break;
            }
