...
| **R2-15** | **Cache + instrumentation DONE 19 September** (`8cd8b37` cache fix promoted; instrumentation `f2a464d` in pipeline): (1) model-cache repairs corrupt/missing entries on open instead of failing fatally — verified 7/7 on Android Chrome 124 and iOS 27 sim (were 4/7); the durable verified marker removed (it served stale-marked corrupt bytes). (2) Photo preparation is now a **reported run**: select/preview/crop stages are named in the record (`photo-select`/`photo-preview`/`photo-crop`), a failure records its exact stage and kind (`errorStage`/`errorKind`), and the error box shows the **report reference** so a tester ties the failure to the staged report. (3) **Anti-eviction**: `storage.persist()` is requested once per session at the first job, and every run records the storage truth (`persisted`, `usageMb`, `quotaMb`) — an eviction story is readable from the report alone. "failed to crop" was never a shipped string; with these records, the next occurrence names itself. **Remaining:** Mac lane `morphVideo` stage iteration, Android emulator e2e over CDP, TrueNAS runner Chrome provisioning + a `runs-on: [self-hosted, truenas]` lane workflow, then the physical-iPhone retest — the required gate for the operator's incident |
...
| **R2-16** | The names surface grid is essentially gap-free — edge-to-edge tiles at container width, no visible gutters at 390 px and 1280 px, images at natural 200px or larger. Picking a name paints the face tile from the catalogue's served image **immediately** (zero synthesis events, e2e asserts `img[src]` = catalogue image and no `synthesis` stage); full-size/latent materialisation continues in the background per the Names row, and the preview upgrades when full-size arrives. Never a blank tile or spinner for bytes already on the device |
