...
| **R2-15** | Photo-path errors carry stage + underlying cause (no bare engine strings). Cache `missing-after-acquire`/repair failures recover for the photo workflow with visible status instead of a fatal box. iOS sim component suite reaches 7/7 on iOS 26.5 and 27.0 (Safari repair-path divergence fixed). The full encode pipeline runs per lane per [local-device-lab.md](local-device-lab.md): Mac real-Chrome lane (5/6 at lab time), Android emulator lane (provisioned; suite 4/7), the **TrueNAS self-hosted GPU runner** (`truenas` label, online 19 Sept — Chrome-for-Testing + WebGPU on the GTX 1050; triton may join later as its own runner), each recording `webgpuExposed`, adapter identity and the admitted route; iPhone-sim full-UI via `safaridriver` runner or the physical iPhone, which stays the required gate |
