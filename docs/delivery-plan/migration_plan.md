> **16 September native desktop GPU requirement:** Tauri must ship first-class native GPU inference independent of browser/WebView GPU support, so a capable desktop GPU remains usable when browser WebGPU fails. CPU-only packages do not complete this phase. Require actual installed-artifact GPU evidence, complete seed/photo/morph workflows, provider reporting, safe CPU fallback and the existing Oliver/Manjaro gate across the declared platform/hardware matrix. See [mandatory desktop GPU delivery](web_checkface_delivery_plan.md#native-desktop-gpu--mandatory-first-class-delivery-requirement-16-september).
