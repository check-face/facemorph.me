...
The Tauri app must provide **actual native GPU acceleration independent of browser/WebView WebGPU or WebGL**. A core reason to offer the desktop app is to help someone whose computer has a capable GPU but cannot run the browser GPU path. A CPU-only desktop artifact is an interim development build and does not complete this phase or qualify as that recommended GPU alternative. This is required implementation work now, not an optional optimization after release.
