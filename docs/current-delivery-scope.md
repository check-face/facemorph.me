...
| Names and public cache | **Amended 18 September:** preserve the names experience as **`next.names.facemorph.me`** (near-identical port; candidate opens it directly, no browse modal; reopening serves each additional name in multi-face sequences) with all 5,055 names. Clients pick from the native 200px grid; selecting a name materialises the face automatically — hosted cache first, device generation never behind a Generate click. Publish verified synthetic seed/name assets including historic lossy full-size WebP/JPEG (same face; compression within tolerance); morph endpoint frames may use the exact stored image; never uploaded/recovered/e4e photos. Cache misses can use device generation. Missing full-size (2,144 names) may be generated at 1024 via the public API on Triton, identity-verified, with latents added as catalogue metadata. |
