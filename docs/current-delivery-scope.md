...
| Photo UX | Tile/+ click, touch and keyboard open the picker; dropping a photo targets that tile. Validate safely, preserve cancelled inputs and explain invalid/multiple-face selections. |
...
| Diagnostics | Optional session opt-in, automatic sanitized saves, visible report reference/status and 30-day expiry. No blanket banner; no private inputs; generation works with reporting off or unavailable. |
