...
test('Diagnostics require fresh opt-in, strip private fields, and ignore old-consent responses',async()=>{
 ...
 d.start('morph');d.stage('synthesis',{elapsedMs:123,photo:'PRIVATE_PHOTO',value:'PRIVATE_WORDS',latent:[42]});assert.equal(posts.length,backlog+2);assert(!JSON.stringify(posts).includes('PRIVATE'));assert(!JSON.stringify(posts).includes('latent'));const prior=posts[0].session;
 ...
 d.start('faces');d.bundle('a'.repeat(64));/* start is deferred until the bundle digest is known */assert.notEqual(posts.at(-1).session,prior);d.enable(false);
});
...
test('Measured stages bypass progress throttling; build and device identity remain bounded and opt-in',async()=>{
 ...
 const ctx=vm.createContext({AbortController,crypto:webcrypto,Date,performance:{now:()=>now},setTimeout:()=>1,clearTimeout(){},localStorage:{getItem:k=>{if(k!=='facemorph-debug-consent-v1')reads++;return stored.get(k);},setItem:(k,v)=>{if(k==='facemorph-debug-consent-v1')consentWrites++;else writes++;stored.set(k,v);},removeItem:k=>{stored.delete(k);}},navigator:{userAgent:'Version/26.1 Safari iPhone',platform:'iPhone',language:'en-AU'},window:{dispatchEvent(){}},CustomEvent:class{},fetch:async(url,request)=>{assert.equal(url,'https://next.facemorph.me/diagnostics/events');assert.equal(request.headers['X-Facemorph-Diagnostics-Consent'],'session-v1');posts.push(JSON.parse(request.body));return {ok:true};}});
 ...
 d.enable(true);const flushed=posts.length;d.bundle('a'.repeat(64));d.start('faces','webgpu');d.stage('asset-acquisition',{loaded:1});d.stage('asset-acquisition',{loaded:2});d.stage('model-loaded',{elapsedMs:12.4,photo:'private'});d.stage('synthesis-complete',{elapsedMs:5.6});
 ...
 assert.match(posts[flushed].device,/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/);assert.equal(writes,1);assert.equal(consentWrites,1);assert.equal(stored.get('facemorph-debug-consent-v1'),'on');assert(!JSON.stringify(posts).includes('private'));d.enable(false);assert.equal(stored.has('facemorph-debug-consent-v1'),false,'Turning reporting off clears the stored opt-in');const count=posts.length;d.stage('encoding-complete',{elapsedMs:10});assert.equal(posts.length,count);
});
