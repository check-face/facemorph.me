...
const allowedStages=new Set(['asset-acquisition','runtime-loading','model-loading','model-loaded','mapping-loading','mapping','canary','synthesis','synthesis-complete','alignment','alignment-complete','encoder-loading','encoder-loaded','encoding','encoding-complete','mapping-complete','original-cache-hit','original-cached','cache-unavailable','codec-loading','face','morph','export']);
let device=null,until=0,session=null,run=null,started=0,last=0,expiry,aborters=new Set();
...
async function send(event){if(!enabled()){off();return;}const sendingSession=session,sendingRun=run,controller=new AbortController();aborters.add(controller);const timer=setTimeout(()=>controller.abort(),10000);try{
 const response=await fetch('/diagnostics/events',{method:'POST',credentials:'omit',headers:{'Content-Type':'application/json'},signal:controller.signal,body:JSON.stringify({schemaVersion:1,session,run,device,...event})});
...
export const diagnostics={
 enable(value){off();if(value){until=Date.now()+60*60*1000;session=crypto.randomUUID();try{device=localStorage.getItem('facemorph-debug-device-v1');if(!/^[0-9a-f-]{36}$/.test(device||'')){device=crypto.randomUUID();localStorage.setItem('facemorph-debug-device-v1',device);}}catch{device=crypto.randomUUID();}expiry=setTimeout(()=>{off();notice('expired');},60*60*1000);notice('enabled');}else notice('disabled');},
 start(action){if(!enabled())return;run=crypto.randomUUID();started=performance.now();last=0;void send({event:'start',action:action==='morph'?'morph':'faces',...environment(),language:navigator.language.replace(/[^A-Za-z-]/g,'').slice(0,20),build:typeof process!=='undefined'?process.env.FACEMORPH_BUILD_ID||'development':'development'});},
 stage(stage,event={}){if(!enabled()||!run||!allowedStages.has(stage))return;const timing=Number.isFinite(event.elapsedMs)?Math.round(event.elapsedMs):undefined;if(timing===undefined&&performance.now()-last<1000)return;last=performance.now();void send({event:'stage',stage,elapsedMs:Math.round(performance.now()-started),stageMs:timing});},
