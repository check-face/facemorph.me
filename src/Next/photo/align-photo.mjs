...
export function createPhotoAligner({manifestUrl='/runtime/photo/manifest.json',workerUrl='/runtime/photo/photo-worker.mjs',workerFactory=url=>new Worker(url,{type:'module'}),stallMs=180000}={}){
 let active=false;
 const alignPhoto=async(blob,{signal,onProgress=()=>{},tryAlign=true}={})=>{
  ...
  try{return await new Promise((resolve,reject)=>{
   ...
   reset();worker.postMessage({type:'align',blob,tryAlign,manifestUrl:new URL(manifestUrl,globalThis.location?.href||import.meta.url).href});
