...
asset cache, without modifying the frozen kernel implementation. The ordinary WebGPUv9 split model and unchanged WGSL resampling kernel are integrated with verified ORT1.22 JSEP assets. Its largest single storage binding is the 128MiB resample output (134217728 bytes), checked on both the adapter and the ORT device before loading, so Android adapters reporting exactly that cap are admitted. The served bundle and this engine are staged from different inputs, so `hosting/next-static/webgpu_contract.py` gates promotion on the split and kernel the engine actually loads; `hosting/next-static/restage-runtime.py --webgpu` rebuilds that section from the research artifacts. Device loss invalidates the route. Numerical semantics tests are distinct from real browser evidence. Browser-only
