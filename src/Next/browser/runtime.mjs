...
export function createBrowserRuntime({manifest: suppliedManifest, manifestSha256:suppliedManifestSha256, manifestUrl='/runtime/manifest.json',onProgress=()=>{},alignPhoto,preferredRoute='auto',workerFactory=()=>new Worker(new URL('./ort-worker.mjs',import.meta.url),{type:'module'}),stallMs=300000}={}){
 ...
 let manifest=suppliedManifest,manifestHash=suppliedManifestSha256,worker,active,disposed=false,validated=false,encoderValidatedSha256,originals,route='cpu',qualificationPending=false;const pending=new Map();let sequence=0;
 ...
 async function start(progress){if(worker)return;await config();worker=workerFactory();worker.onmessage=({data})=>{const task=pending.get(data.id);if(!task)return;if(data.type==='progress'){task.reset();emit(data,task.progress);return;}clearTimeout(task.timer);pending.delete(data.id);unmark();if(data.type==='error'){if(task.operation==='encode-aligned')encoderValidatedSha256=undefined;validated=false;failedRoutes.add(route);const error=Object.assign(Error(data.error.message),{name:data.error.name,failedOperation:task.operation,correctnessFailure:data.error.correctnessFailure===true});stop(error);task.reject(error);}else task.resolve(data.result);};worker.onerror=()=>stop(Error('The local generation engine stopped.'));await send('initialize',{manifest,provider:route==='webgl'?'webgl2':route==='webgpu'?'webgpu':'wasm',manifestSha256:manifestHash},progress);}
 ...
 // to pass its canaries.
 const SPEED_KEY='facemorph-route-speed-v1';
 function speeds(){try{const raw=JSON.parse(localStorage.getItem(SPEED_KEY)||'{}');return raw&&raw.bundle===manifestHash&&raw.routes?raw.routes:{};}catch{return {};}}
 function remember(name,ms){try{const routes={...speeds(),[name]:Math.round(ms)};localStorage.setItem(SPEED_KEY,JSON.stringify({bundle:manifestHash,routes}));}catch{}}
 // Only the admitted route gets timed, so a device that starts on a slow route would never learn
 // that another is quicker. When the routes measured so far are all slow, the next run spends one
 // admission measuring an untried one; after that the fastest measured route simply wins. The
 ...
 async function admit(progress,signal,forceCpu=false){
  ...
  try{const began=Date.now();await start(progress);const result=await send('qualify',{},progress);validated=result.deviceValidated;qualificationPending=result.partial===true;remember(route,Date.now()-began);announce(progress);}
  catch(error){if(signal.aborted)throw signal.reason;
   if(route==='cpu')throw error;
   const unsupportedWebgpu=route==='webgpu'&&error.name==='NotSupportedError';
   refused(progress,route,unsupportedWebgpu?'unsupported':'canary-failed');
   // An explicitly selected route has nothing to fall back to by definition, but the refusal
   // is still named before the run fails (C-02).
   if(preferredRoute!=='auto'){failedRoutes.add(route);stop();throw error;}
   failedRoutes.add(route);stop();if(unsupportedWebgpu&&manifest.webgl&&!failedRoutes.has('webgl')){route='webgl';try{await start(progress);const result=await send('qualify',{},progress);validated=result.deviceValidated;qualificationPending=result.partial===true;announce(progress);return;}catch(glError){if(signal.aborted)throw signal.reason;failedRoutes.add('webgl');stop();}}emit({stage:'fallback-cpu'},progress);route='cpu';await start(progress);const result=await send('qualify',{},progress);validated=result.deviceValidated;qualificationPending=result.partial===true;announce(progress);}
 }
 ...
 qualify:(requestedRoute='cpu',options={})=>afterDelivery(operation(async progress=>{if(!['cpu','webgl','webgpu'].includes(requestedRoute))throw Error('Choose CPU, WebGL or WebGPU.');if(route!==requestedRoute)stop();route=requestedRoute;await start(progress);const result=await send('qualify',{},progress);validated=result.deviceValidated;qualificationPending=result.partial===true;return result;},options)),
 generate:(request,options={})=>afterDelivery(operation(async(progress,signal)=>{const {identity}=await inputLatent(request.mode,request.value);return cachedGenerate({kind:'seed',identity},async()=>{await start(progress);return send('generate',{mode:request.mode,value:request.value},progress);},progress,signal);},{...request,...options})),
