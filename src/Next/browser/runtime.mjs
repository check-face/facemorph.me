...
export function createBrowserRuntime({manifest: suppliedManifest, manifestSha256:suppliedManifestSha256, manifestUrl='/runtime/manifest.json',onProgress=()=>{},alignPhoto,preferredRoute='auto',workerFactory=()=>new Worker(new URL('./ort-worker.mjs',import.meta.url),{type:'module'}),stallMs=300000}={}){
 ...
 async function admit(progress,signal,forceCpu=false){
  ...
  try{const began=Date.now();await start(progress);const result=await send('qualify',{},progress,ACQUIRE_STALL);validated=result.deviceValidated;qualificationPending=result.partial===true;remember(route,Date.now()-began);announce(progress);}
   ...
   // Fallback invariant (operator direction, 19 September): never fall back to a route whose
   // recorded cost is higher than another route still available. The next attempt is the
   // fastest untried route per the measured priors - on the S24 that is CPU (2.6s) before
   // WebGL (13s), never the reverse. Every tried route keeps its refused-route announcement.
   for(const next of priorOrder(capability(),supportedRoutes().filter(name=>!failedRoutes.has(name)))){
    route=next;
    try{await start(progress);const result=await send('qualify',{},progress,ACQUIRE_STALL);validated=result.deviceValidated;qualificationPending=result.partial===true;announce(progress);return;}
    catch(retryError){if(signal.aborted)throw signal.reason;failedRoutes.add(route);stop();}
   }
   throw error;}
 }
