...
export function createBrowserRuntime({manifest: suppliedManifest, manifestSha256:suppliedManifestSha256, manifestUrl='/runtime/manifest.json',onProgress=()=>{},alignPhoto,preferredRoute='auto',workerFactory=()=>new Worker(new URL('./ort-worker.mjs',import.meta.url),{type:'module'}),stallMs=300000}={}){
 ...
 async function admit(progress,signal,forceCpu=false){
  ...
  try{const began=Date.now();await start(progress);const result=await send('qualify',{},progress,ACQUIRE_STALL);validated=result.deviceValidated;qualificationPending=result.partial===true;remember(route,Date.now()-began);announce(progress);}
   ...
   failedRoutes.add(route);stop();if(unsupportedWebgpu&&manifest.webgl&&!failedRoutes.has('webgl')){route='webgl';try{await start(progress);const result=await send('qualify',{},progress,ACQUIRE_STALL);validated=result.deviceValidated;qualificationPending=result.partial===true;announce(progress);return;}catch(glError){if(signal.aborted)throw signal.reason;failedRoutes.add('webgl');stop();}}emit({stage:'fallback-cpu'},progress);route='cpu';await start(progress);const result=await send('qualify',{},progress,ACQUIRE_STALL);validated=result.deviceValidated;qualificationPending=result.partial===true;announce(progress);}
 }
