// Product extraction of ordinary gpu-worker-v9 / mod-fusion. No kernel changes.
export async function createWebGpuSession({config,noiseManifest,bytes,progress=()=>{}}){
 ...
 const ort=await import(/* webpackIgnore: true */ runtimeUrl);ort.env.wasm.numThreads=1;ort.env.wasm.wasmPaths={mjs:factoryUrl,wasm:wasmUrl};
 ...
 try{
  ...
  const kernelUrl=await moduleUrl(config.kernel);const {fusedResample}=await import(/* webpackIgnore: true */ kernelUrl);
  const fused=await fusedResample(device,{phase:outputs[meta.phase].buffer,demod:outputs[meta.demod]?.buffer,noise:all.noise_15.buffer,filter:filterBuffer.buffer,bias:biasBuffer.buffer,output:external.buffer},meta,true,256);
