...
with urllib.request.urlopen(ORIGIN+'/runtime/manifest.json',timeout=60) as r: manifest=r.read(4*1024*1024+1)
...
class Handler(SimpleHTTPRequestHandler):
 ...
 def end_headers(self):
  for k,v in [('Cross-Origin-Opener-Policy','same-origin'),('Cross-Origin-Embedder-Policy','require-corp'),('Cross-Origin-Resource-Policy','same-origin'),('Cache-Control','no-store')]:self.send_header(k,v)
  super().end_headers()
 ...
 def do_GET(self):
  ...
  try:
   if self.path=='/runtime/manifest.json':data=manifest;mime='application/json'
    ...
    with urllib.request.urlopen(ORIGIN+self.path,timeout=180) as r:
