...
def stage_morphVideo():
 ...
 js("window.__ciPlayback={done:false};(async()=>{const deadline=new Promise((_,j)=>setTimeout(()=>j(Error('playback probe exceeded 9s')),9000));try{const v=document.querySelector('.next-video-slot video');v.muted=true;await Promise.race([v.play(),deadline]);await Promise.race([new Promise(r=>{v.requestVideoFrameCallback(()=>r());}),deadline]);window.__ciPlayback={done:true,width:v.videoWidth,height:v.videoHeight,duration:v.duration,time:v.currentTime};v.pause();}catch(e){const v=document.querySelector('.next-video-slot video');window.__ciPlayback={done:true,error:String(e),src:(v&&v.src||'').slice(0,60),readyState:v?v.readyState:null,networkState:v?v.networkState:null,mediaError:v&&v.error?v.error.code+':'+(v.error.message||''):null,duration:v?v.duration:null};}})()")
