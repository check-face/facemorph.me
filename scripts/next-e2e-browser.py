...
def stage_morphVideo():
 ...
 js("window.__ciPlayback={done:false};(async()=>{const deadline=new Promise((_,j)=>setTimeout(()=>j(Error('playback probe exceeded 30s')),30000));try{const v=document.querySelector('.next-video-slot video');v.muted=true;await Promise.race([v.play(),deadline]);await Promise.race([new Promise((r,j)=>{v.requestVideoFrameCallback(()=>r());}).then(()=>{}),deadline]);window.__ciPlayback={done:true,width:v.videoWidth,height:v.videoHeight,duration:v.duration,time:v.currentTime};v.pause();}catch(e){window.__ciPlayback={done:true,error:String(e)};}})()")
